...
function HowItworks() {
  ...
  useEffect(() => {
    ...
    const updateParallax = () => {
      ...
      const offset = Math.max(
        -34,
        Math.min(34, (viewportCenter - frameCenter) * 0.08),
      );
  ...
  return (
    <section id="how-it-works" ref={sectionRef} className="bg-black relative">
      <Image
      ...
      />
      <div className="relative px-4 py-20 sm:p-10 lg:p-14 xl:p-20 2xl:container 2xl:mx-auto  space-y-10 md:space-y-7 xl:space-y-10">
        <div
          className={`relative z-10 how-it-works-header-animation max-w-100 space-y-2 lg:max-w-130 lg:space-y-2.5 xl:max-w-171.5 xl:space-y-4 ${hasEntered ? "is-visible" : ""}`}
        >
          ...
          <p className="text-surface text-base lg:text-lg xl:text-2xl leading-6 md:leading-5 lg:leading-6.5 xl:leading-9">
            No manual data entry. Point Almari at where your clothes already
            live, and it does the cataloguing.
          </p>
        </div>
        ...
        <div className="relative z-10 bg-[#1b1a1a] p-1 md:p-1.5 xl:p-2 space-y-1.5 xl:space-y-2">
          <div
            ref={imageFrameRef}
            className="overflow-hidden bg-foreground p-1.5 xl:p-2"
          >
            <Image
              ...
              style={{
                transform: `translateY(${imageParallax}px) scale(1.08)`,
              }}
            />
          </div>
          ...
          <div className="grid gap-1 md:gap-1.5 xl:gap-2 bg-[#1b1a1a] grid-cols-1 md:grid-cols-3 ">
            {howItWorksSteps.map((step) => (
              <article
                key={step.title}
                className="bg-foreground p-6 md:p-2.5 lg:p-4 xl:p-6 space-y-2"
              >
